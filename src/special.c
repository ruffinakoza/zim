/*
 * $Id: special.c 933 2006-11-19 22:37:00Z zsuzsu $
 */
...
DECLARE_SPEC_FUN(spec_cast_clan_mage	);
DECLARE_SPEC_FUN(spec_cast_clan_cleric	);
...
const   struct  spec_type    spec_table[] =
{
	...
	{ "spec_cast_clan_cleric",	spec_cast_clan_cleric	},
	{ "spec_cast_clan_mage",	spec_cast_clan_mage	},
...
bool spec_cast_mage(CHAR_DATA *ch)
{
	...
	for (;;) {
		...
		default: min_level = 45; spell = "acid blast";     break;
		}
...
bool spec_executioner(CHAR_DATA *ch)
{
	...
	for (victim = ch->in_room->people; victim; victim = v_next) {
		...
		if (!IS_NPC(victim) && IS_SET(victim->state_flags, STATE_WANTED) 
		&&  can_see(ch, victim)) {
			crime = "CRIMINAL";
			break;
		}
...
bool spec_janitor(CHAR_DATA *ch)
{
	...
	for (trash = ch->in_room->contents; trash != NULL; trash = trash_next) {
		...
		||  !can_loot(ch, trash) 
		|| trash->pIndexData->item_type == ITEM_QUESTFIND
...
bool spec_guard(CHAR_DATA *ch)
{
	...
	for (victim = ch->in_room->people; victim != NULL; victim = v_next) {
		...
		if (number_percent() < 2 && !IS_IMMORTAL(victim) && !IS_NPC(victim)) {
			if (HAS_SKILL(victim, gsn_ruler_badge)) {
				act("$n stands at attention and salutes you.",
					ch, NULL, victim, TO_VICT);
				act("$n stands at attention and salutes $N.",
					ch, NULL, victim, TO_NOTVICT);
			}
			else {
				...
				if (str_cmp(ch->in_room->area->name,
					...
					do_say(ch, "Oh, my dear! I have just remembered.");
					interpret(ch, "smile");
				}
			}
		}
		...
		&&  victim->fighting != ch) {
			if (HAS_SKILL(victim, gsn_ruler_badge))
				ech = victim->fighting;
			else if (HAS_SKILL(victim->fighting, gsn_ruler_badge))
				ech = victim;
			else if (victim->alignment < victim->fighting->alignment)
				ech = victim;
			else if (victim->fighting->alignment < victim->alignment)
				ech = victim->fighting;
			else if (IS_GOOD(victim) && IS_GOOD(victim->fighting))
				ech = NULL;
			else 
				ech = (victim->level > victim->fighting->level)
...
static void spec_cast(CHAR_DATA *ch, const char *spell_name, CHAR_DATA *victim)
{
	...
	&&  (target == TAR_CHAR_OFFENSIVE || target == TAR_OBJ_CHAR_OFF)) {
		if (IS_SET(ch->pIndexData->act, ACT_CLERIC))
			doprintf(interpret, ch, "commune '%s'", spell_name);
		else
	...
	if (IS_SET(ch->pIndexData->act, ACT_CLERIC))
		doprintf(interpret, ch, "commune '%s' %d.'%s'", spell_name, num, name);
	else
		doprintf(interpret, ch, "cast '%s' %d.'%s'", spell_name, num, name);
}

bool spec_cast_clan_mage(CHAR_DATA *ch)
{
	CHAR_DATA *victim;
	CHAR_DATA *v_next;
	char *spell;
	...
	case  3: spell = "weaken";      break;
	case  4: spell = "acid arrow";  break;
	case  5: spell = "fireball";    break;
	case  6: spell = "energy drain";break;
	case  7: spell = "plague";	break;
	case  8: spell = "lightning breath";  break;
	case  9: spell = "teleport";	break;
	case 10: spell = "faerie fire"; break;
	case 11: spell = "fear";	break;
	case 12:
	case 13:
	case 14:
	case 15:
	case 16:
	case 17: spell = "acid blast";	break;
	default: return FALSE;
	}
...
bool spec_cast_clan_cleric(CHAR_DATA *ch)
{
	CHAR_DATA *victim;
	CHAR_DATA *v_next;
	char *spell;
	...
	case  2: spell = "dispel magic";	break;
	case  3: spell = "weaken";      	break;
	case  4: spell = "flamestrike";  	break;
	case  5: if (IS_EVIL(ch))	spell = "demonfire";
		 else if (IS_GOOD(ch)) 	spell = "wrath";
		 else 			spell = "bluefire";
		 break;
	case  6: if (IS_EVIL(ch))	spell = "demonfire";
		 else if (IS_GOOD(ch)) 	spell = "holy word";
		 else 			spell = "wrath";
		 break;
	case  7: if (IS_EVIL(ch))	spell = "turn";
		 else if (IS_GOOD(ch)) 	spell = "turn";
		 else 			spell = "turn";
		 break;
	case  8: spell = "energy drain";	break;
	case  9: spell = "teleport";		break;
	case 10: spell = "faerie fire"; 	break;
	case 11: spell = "blade barrier";	break;
	case 12:
	case 13:
	case 14:
	case 15:
	case 16:
	case 17: if (IS_EVIL(ch))	spell = "demonfire";
		 else if (IS_EVIL(victim))spell = "ray of truth";
		 else 			spell = "flamestrike";
		 break;
	default: return FALSE;
	}
