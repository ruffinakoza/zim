/*
 * $Id: recycle.c 998 2007-01-21 21:23:49Z zsuzsu $
 */
...
OBJ_DATA *new_obj(void)
{
	...
	obj->name		= str_empty;
	obj->material		= MATERIAL(MAT_UNKNOWN);
	obj->material_descr	= str_empty;
	obj->augment		= NULL;

	for (i =0; i < PAST_OWNER_MAX; i++) {
		obj->past_owner[i] = NULL;
		obj->past_owner_ip[i] = NULL;
	}
...
void free_obj(OBJ_DATA *obj)
{
	...
	for (i = 0; i < PAST_OWNER_MAX; i++) {
		free_string(obj->past_owner[i]);
		obj->past_owner[i] = NULL;
		free_string(obj->past_owner_ip[i]);
		obj->past_owner_ip[i] = NULL;
	}
	...
	mlstr_free(obj->writtenDesc);
	obj->writtenDesc = NULL;

	mlstr_free(obj->owner);
	obj->owner = NULL;

	mlstr_free(obj->killer);
	obj->killer = NULL;

	obj->material = MATERIAL(MAT_UNKNOWN);
	free_string(obj->material_descr);
	obj->material_descr = NULL;
...
CHAR_DATA *new_char (void)
{
	...
	ch->name		= str_empty;
	ch->prefix		= str_empty;
	ch->material		= MATERIAL(MAT_UNKNOWN);
	ch->material_descr	= str_empty;
	ch->prompt		= str_empty;
	ch->lines		= SCROLL_MAX;
	ch->logon		= current_time;
	ch->comm		= COMM_COLOR;
	ch->fixed_flags		= 0;
	ch->hit = ch->max_hit	= CHAR_CREATE_INITIAL_HP;
	ch->mana = ch->max_mana	= CHAR_CREATE_INITIAL_MANA;;
	ch->move = ch->max_move	= CHAR_CREATE_INITIAL_MOVE;
...
void free_char(CHAR_DATA *ch)
{
	...
	ch->prefix = NULL;
 	...
 	ch->material = MATERIAL(MAT_UNKNOWN);
	free_string(ch->material_descr);
	ch->material_descr = NULL;

	free_string(ch->in_mind);
	ch->in_mind = NULL;

	if (ch->pcdata != NULL)
		ch->pcdata->pdata->online = NULL;
...
PC_DATA *new_pcdata(void)
{
	...
	pcdata->pk_ok_list = str_empty;
	pcdata->granted = str_empty;
	pcdata->enslaver = str_empty;
	pcdata->born = current_time;

	for (i=0; i < ALIGN_INDEX_MAX; i++)
		pcdata->align_killed[i] = 0;
...
void free_pcdata(PC_DATA *pcdata)
{
	...
	free_string(pcdata->pk_ok_list);
	free_string(pcdata->granted);
	free_string(pcdata->enslaver);
	buf_free(pcdata->buffer);
    
	for (obj = pcdata->medals; obj; obj = obj_next) {
		obj_next = obj->next_content;
		extract_obj(obj, XO_F_NOCOUNT);
	}

	for (obj = pcdata->awards; obj; obj = obj_next) {
		obj_next = obj->next_content;
		extract_obj(obj, XO_F_NOCOUNT);
	}
