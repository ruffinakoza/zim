/*
 * $Id: fight.h 915 2006-10-11 22:37:10Z zsuzsu $
 */
...
#define raw_kill(ch, victim, is_duel) raw_kill_org(ch, victim, -1, is_duel)
...
int	tohit_chance		(CHAR_DATA *ch, CHAR_DATA *victim,
				int dt, int loc);

#define DAMF_NONE	(0)
#define DAMF_SHOW	(A)	/* show dam message */
#define DAMF_SECOND	(B)	/* damage inflicted by second weapon */
#define DAMF_NOREDUCTION (C)	/* no sanctuary etc reduction */

#define DEATH_DUEL       0
#define DEATH_SLAY       1
#define DEATH_KILL       2
#define DEATH_ENSLAVED   3
