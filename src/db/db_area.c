/*
 * $Id: db_area.c 936 2006-11-20 04:50:44Z zsuzsu $
 */
...
DBLOAD_FUN(load_helps)
{
	...
	for (; ;) {
		...
		/*
		 * for backward compatibility "possible"
		 * is being used.
		 * */
		fread_possible_number(fp, &clan);
...
DBLOAD_FUN(load_old_mob)
{
	...
	for (; ;) {
		...
		if (name[0] == '\0' || (race =  rn_lookup(name)) == -1) {
			race_t *r;
...
DBLOAD_FUN(load_old_obj)
{
	...
	for (; ;) {
		...
		pObjIndex->material_descr= str_dup("copper");
        	pObjIndex->material = MATERIAL(material_lookup_name(pObjIndex->material_descr));
	...
	for (; ;) {
		...
		for (; ;) {
			...
			if (letter == 'H') /* healing room */
				...
				/* if levels are present */
				if (fread_possible_number(fp, &pexit->lock_level))
						pexit->trap_level = fread_number(fp);
...
DBLOAD_FUN(load_mobiles)
{
	...
	if (pMobIndex->race < 0) pMobIndex->race = 0;
	r = RACE(pMobIndex->race);
	...
	tmp = fread_word(fp);
	if (atoi(tmp) > 0)
		pMobIndex->dam_type = atoi(tmp);
	else
		pMobIndex->dam_type = attack_lookup(tmp) | r->dam_type;
	...
	/* size */
	pMobIndex->size			= fread_fword(size_table, fp) | r->size;
	pMobIndex->material = MATERIAL(material_lookup_name(tmp2 = fread_string(fp)));
	free_string(tmp2);
	pMobIndex->material_descr	= fread_string(fp);
	...
	for (; ;)
		...
		else if (!str_prefix(word,"attr+"))
		    SET_BIT(pMobIndex->attr_flags,vector);
...
DBLOAD_FUN(load_objects)
{
	...
	newobjs++;
        ...
        pObjIndex->short_descr		= mlstr_fread(fp);
        pObjIndex->description		= mlstr_fread(fp);
	pObjIndex->material = MATERIAL(material_lookup_name(tmp = fread_string(fp)));
	free_string(tmp);
        pObjIndex->material_descr	= fread_string(fp);
	...
	{
	case ITEM_ARMOR:
	    pObjIndex->value[ITEM_ARMOR_AC_PIERCE]	= fread_number(fp);
	    pObjIndex->value[ITEM_ARMOR_AC_BASH]	= fread_number(fp);
	    pObjIndex->value[ITEM_ARMOR_AC_SLASH]	= fread_number(fp);
	    pObjIndex->value[ITEM_ARMOR_AC_EXOTIC]	= fread_number(fp);
	    pObjIndex->value[ITEM_ARMOR_TYPE]		= flag_value(armor_types, fread_word(fp));
	    pObjIndex->value[ITEM_ARMOR_FLAGS]		= fread_flags(fp);
	    break;

	case ITEM_MONEY:
	    pObjIndex->value[ITEM_MONEY_GOLD]		= fread_number(fp);
	    pObjIndex->value[ITEM_MONEY_SILVER]		= fread_number(fp);
	    break;
	
	case ITEM_WEAPON:
	    pObjIndex->value[ITEM_WEAPON_TYPE]		= flag_value(weapon_class,
						     fread_word(fp));
	    pObjIndex->value[ITEM_WEAPON_DICE_NUM]	= fread_number(fp);
	    pObjIndex->value[ITEM_WEAPON_DICE_SIZE]	= fread_number(fp);
	    pObjIndex->value[ITEM_WEAPON_ATTACK_TYPE]	= attack_lookup(fread_word(fp));
	    pObjIndex->value[ITEM_WEAPON_FLAGS]		= fread_flags(fp);
	    break;

	case ITEM_CONTAINER:
	    pObjIndex->value[ITEM_CONTAINER_WEIGHT]	= fread_number(fp);
	    pObjIndex->value[ITEM_CONTAINER_FLAGS]	= fread_flags(fp);
	    pObjIndex->value[ITEM_CONTAINER_KEY]	= fread_number(fp);
	    pObjIndex->value[ITEM_CONTAINER_PER_ITEM_WEIGHT]= fread_number(fp);
	    pObjIndex->value[ITEM_CONTAINER_WEIGHT_MULTI]= fread_number(fp);
	    pObjIndex->value[ITEM_CONTAINER_QUANTITY]	= fread_number(fp);
	    break;

        case ITEM_DRINK_CON:
	case ITEM_FOUNTAIN:
            pObjIndex->value[ITEM_DRINK_TOTAL]		= fread_number(fp);
            pObjIndex->value[ITEM_DRINK_REMAINING]	= fread_number(fp);
	    pObjIndex->value[ITEM_DRINK_TYPE]		= liq_lookup(fread_word(fp));
            pObjIndex->value[ITEM_DRINK_POISON]		= fread_number(fp);
            break;

	case ITEM_WAND:
	case ITEM_STAFF:
	    pObjIndex->value[ITEM_WAND_LEVEL]		= fread_number(fp);
	    pObjIndex->value[ITEM_WAND_CHARGES_TOTAL]	= fread_number(fp);
	    pObjIndex->value[ITEM_WAND_CHARGES_REMAINING]= fread_number(fp);
	    pObjIndex->value[ITEM_WAND_SPELL]		= sn_lookup(fread_word(fp));
	    break;

	case ITEM_POTION:
	case ITEM_PILL:
	case ITEM_SCROLL:
 	    pObjIndex->value[ITEM_POTION_LEVEL]		= fread_number(fp);
	    pObjIndex->value[ITEM_POTION_SPELL1]	= sn_lookup(fread_word(fp));
	    pObjIndex->value[ITEM_POTION_SPELL2]	= sn_lookup(fread_word(fp));
	    pObjIndex->value[ITEM_POTION_SPELL3]	= sn_lookup(fread_word(fp));
	    pObjIndex->value[ITEM_POTION_SPELL4]	= sn_lookup(fread_word(fp));
	    break;

	case ITEM_PORTAL:
            pObjIndex->value[ITEM_PORTAL_CHARGES]	= fread_number(fp);
            pObjIndex->value[ITEM_PORTAL_EXIT_FLAGS]	= fread_flags(fp);
            pObjIndex->value[ITEM_PORTAL_FLAGS]		= fread_flags(fp);
            pObjIndex->value[ITEM_PORTAL_DEST]		= fread_number(fp);
            pObjIndex->value[ITEM_PORTAL_KEY]		= fread_number(fp);
	    break;

	case ITEM_LIGHT:
            pObjIndex->value[ITEM_LIGHT_DURATION]	= fread_number(fp);
	    break;
	...
	default:
            ...
            pObjIndex->value[3]             = fread_flags(fp);
	    pObjIndex->value[4]		    = fread_flags(fp);
	    pObjIndex->value[5]		    = fread_flags(fp);
	    pObjIndex->value[6]		    = fread_flags(fp);
	    break;
	}
...
/*Zz*/
		switch (gcount) {
			case 0: pObjIndex->rsex = fread_fword(rsex_flags, fp);
				break;
			case 1: pObjIndex->rrace = fread_fword(rrace_flags, fp);
				break;
			case 2: pObjIndex->rclass = fread_fword(rclass_flags, fp);
				break;
			case 3: pObjIndex->rsize = fread_fword(rsize_flags, fp);
				break;
		}
		gcount++;
	    }
...
void convert_mobile(MOB_INDEX_DATA *pMobIndex)
{
	...
	pMobIndex->material_descr    = str_dup(str_empty);
