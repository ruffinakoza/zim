/*
 * $Id: db.c 1019 2007-02-15 00:52:41Z zsuzsu $
 */
...
int			reboot_counter;
flag32_t		world_affects;
flag32_t		global_channels = ALL;
...
void boot_db(void)
{
	...
	/* reboot counter */
	reboot_counter = 7*1440;	/* 4 x 24 hours */
	...
	LOG("loading: character persistant data");
	load_all_ch_pdata();

	LOG("loading: server usage");
	if (!load_server_usage()) {
		LOG("Couldn't load server usage, approximating from pdata");
		approximate_server_usage_from_pdata();
		save_server_usage();
	}

	LOG("converting old ROM objects");
	...
	LOG("loaded %d world affects", load_waffs());
}
...
void print_resetmsg(AREA_DATA *pArea)
{
	...
	for (d = descriptor_list; d != NULL; d = d->next) {
		...
		if (!ch) {
			BUG("print_resetmsg: char is null? (skipping) %s",
				d->host);
			continue;
		}
...
void area_update(void)
{
	...
	for (pArea = area_first; pArea != NULL; pArea = pArea->next) {
		...
		||  pArea->age >= 31) {
			...
			/* don't update clan area if there's an interloper */
			if (pArea->clan != CLAN_FREEMAN 
			&& (pArea->interlopers != 0
			|| !clan_item_ok(pArea->clan)))
				continue;
...
int area_interloper_reset(AREA_DATA *pArea)
{
	ROOM_INDEX_DATA *pRoom;
	CHAR_DATA *gch;
	int vnum;
	...
	for (vnum = pArea->min_vnum; vnum <= pArea->max_vnum; vnum++) {
        	if ((pRoom = get_room_index(vnum))) {
			gch = pRoom->people;
			for (gch = pRoom->people; gch; gch = gch->next) {
				if (!IS_IMMORTAL(gch)
				&& gch->clan > 0 && gch->clan != pArea->clan) {
					pArea->interlopers++;
					count++;
				}
			}
...
CHAR_DATA *create_mob(MOB_INDEX_DATA *pMobIndex)
{
	...
	mob->material_descr	= str_qdup(pMobIndex->material_descr);
	...
	for (i = 0; i < MAX_STATS; i ++)
		mob->perm_stat[i] = UMAX(40, 40 + mob->level);
	...
	if (IS_SET(pMobIndex->act, ACT_THIEF)) {
		mob->perm_stat[STAT_DEX] += 20;
		mob->perm_stat[STAT_INT] += 5;
		mob->perm_stat[STAT_WIS] -= 5;
	}
		
	if (IS_SET(pMobIndex->act, ACT_CLERIC)) {
		mob->perm_stat[STAT_WIS] += 20;
		mob->perm_stat[STAT_DEX] -= 5;
		mob->perm_stat[STAT_STR] += 5;
	}
		
	if (IS_SET(pMobIndex->act, ACT_MAGE)) {
		mob->perm_stat[STAT_INT] += 20;
...
void clone_mob(CHAR_DATA *parent, CHAR_DATA *clone)
{
	...
	clone->material_descr	= str_qdup(parent->material_descr);
...
OBJ_DATA *create_obj(OBJ_INDEX_DATA *pObjIndex, int flags)
{
	...
	obj->material_descr	= str_qdup(pObjIndex->material_descr);
...
/*
 * Tries to read a number from the file.
 *
 * If a number is not found as the next entry
 * then FALSE is returned and 'num' is 
 * set to 0 and the last character on the stream
 * is pushed back to the file.
 * Otherwise TRUE is returned and the 'num'
 * is set.
 *
 * This has the side-effect of eating whitespace.
 *
 * by Zsuzsu
 */
int fread_possible_number(FILE *fp, int *num)
{
	int number;
	bool sign;
	char c;
...
/*
 * Go through the pfiles and read them to populate the memory
 * resident persistent data such as:
 *
 *    char_pdata objects
 *    Add the objects in players not logged on to object count 
 *
 */
/*void load_limited_objects() -- old name */
void load_limited_objects()
{
	...
	for (dp = readdir(dirp); dp != NULL; dp = readdir(dirp)) {
		...
		for (letter = fread_letter(fp); letter != EOF;
						letter = fread_letter(fp)) {

			BUG("pfile: %s letter %c", dp->d_name, letter);
			switch (letter) {
			case '#':
				word = fread_word(fp);
			...
			case 'P':
				...
				if (!str_cmp(word, "K_Kills")) {
					if (pname == NULL) {
						BUG("load_all_ch_pdata: "
						    "PK_Kills before Name "
						    "in pfile: %s", dp->d_name);
						exit(1);
					}

					rating_add_kill(pname, fread_number(fp));
				}	
				else if (!str_cmp(word, "K_Deaths")) {
					if (pname == NULL) {
						BUG("load_all_ch_pdata: "
						    "PK_Deaths before Name "
						    "in pfile: %s", dp->d_name);
						exit(1);
					}

					rating_add_death(pname, fread_number(fp));
				}	
				break;
...
int fread_skilltree(FILE *fp)
{
        int st;
        const char *name;

        name = fread_string(fp);
        st = skillt_lookup(name);
        if (st < 0) {
                db_error("fread_skilltree", "%s: unknown tree", name);
                st = 0;
        }
        free_string(name);
        return st;
