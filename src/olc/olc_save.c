/*
 * $Id: olc_save.c 981 2006-12-09 15:53:24Z zsuzsu $
 */
...
void save_mobile(FILE *fp, MOB_INDEX_DATA *pMobIndex)
{
    ...
    fprintf(fp, "%s~\n",	pMobIndex->material->name);
    fprintf(fp, "%s~\n",	
    	!IS_NULLSTR(pMobIndex->material_descr)
	&& str_cmp(pMobIndex->material->name, pMobIndex->material_descr)
	&& str_cmp("unknown", pMobIndex->material_descr)
	? pMobIndex->material_descr : "");

    /*new attribute for ZiM */
    if (pMobIndex->attr_flags != 0)
     	fprintf(fp, "F attr+ %s\n", format_flags(pMobIndex->attr_flags));
...
void save_object(FILE *fp, OBJ_INDEX_DATA *pObjIndex)
{
    ...
    fwrite_string(fp, NULL,	pObjIndex->material->name);
    fwrite_string(fp, NULL,	
    	!IS_NULLSTR(pObjIndex->material_descr)
	&& str_cmp(pObjIndex->material->name, pObjIndex->material_descr)
	&& str_cmp("unknown", pObjIndex->material_descr)
	? pObjIndex->material_descr : "");
    ...
    switch (pObjIndex->item_type)
    {
	    ...
	    fprintf(fp, "%s %s %s %s %s %s %s\n",
			format_flags(pObjIndex->value[0]),
	    		...
	    		format_flags(pObjIndex->value[5]),
	    		format_flags(pObjIndex->value[6])
			);
	    break;

	case ITEM_ARMOR:
	    fprintf(fp, "%d %d %d %d '%s' %s\n",
			pObjIndex->value[ITEM_ARMOR_AC_PIERCE],
	    		pObjIndex->value[ITEM_ARMOR_AC_BASH],
	    		pObjIndex->value[ITEM_ARMOR_AC_SLASH],
	    		pObjIndex->value[ITEM_ARMOR_AC_EXOTIC],
	    		flag_string(armor_types, pObjIndex->value[ITEM_ARMOR_TYPE]),
			format_flags(pObjIndex->value[ITEM_ARMOR_FLAGS])
			);
		break;

	case ITEM_MONEY:
	    fprintf(fp, "%d %d\n",
			pObjIndex->value[ITEM_MONEY_SILVER],
	    		pObjIndex->value[ITEM_MONEY_GOLD]
			);
		break;
        ...
        case ITEM_FOUNTAIN:
            fprintf(fp, "%d %d '%s' %d\n",
                     pObjIndex->value[ITEM_DRINK_TOTAL],
                     pObjIndex->value[ITEM_DRINK_REMAINING],
                     liq_table[pObjIndex->value[ITEM_DRINK_TYPE]].liq_name,
		     pObjIndex->value[ITEM_DRINK_POISON]
		     );
            break;

        case ITEM_CONTAINER:
            fprintf(fp, "%d %s %d %d %d %d\n",
                     pObjIndex->value[ITEM_CONTAINER_WEIGHT],
                     format_flags(pObjIndex->value[ITEM_CONTAINER_FLAGS]),
                     pObjIndex->value[ITEM_CONTAINER_KEY],
                     pObjIndex->value[ITEM_CONTAINER_PER_ITEM_WEIGHT],
                     pObjIndex->value[ITEM_CONTAINER_WEIGHT_MULTI],
		     pObjIndex->value[ITEM_CONTAINER_QUANTITY]
		     );
            break;

        case ITEM_WEAPON:
            fprintf(fp, "%s %d %d %s %s\n",
                     flag_string(weapon_class, pObjIndex->value[ITEM_WEAPON_TYPE]),
                     pObjIndex->value[ITEM_WEAPON_DICE_NUM],
                     pObjIndex->value[ITEM_WEAPON_DICE_SIZE],
                     attack_table[pObjIndex->value[ITEM_WEAPON_ATTACK_TYPE]].name,
                     format_flags(pObjIndex->value[ITEM_WEAPON_FLAGS]));
		break;
...
/* no negative numbers */
		     pObjIndex->value[ITEM_POTION_LEVEL] > 0 
		     	? pObjIndex->value[ITEM_POTION_LEVEL] : 0,
		     pObjIndex->value[ITEM_POTION_SPELL1] != -1
			? skill_name(pObjIndex->value[ITEM_POTION_SPELL1]) 
			: str_empty,
		     pObjIndex->value[ITEM_POTION_SPELL2] != -1 
		     	? skill_name(pObjIndex->value[ITEM_POTION_SPELL2]) 
			: str_empty,
		     pObjIndex->value[ITEM_POTION_SPELL3] != -1 
		     	? skill_name(pObjIndex->value[ITEM_POTION_SPELL3]) 
			: str_empty,
		     pObjIndex->value[ITEM_POTION_SPELL4] != -1 
		     	? skill_name(pObjIndex->value[ITEM_POTION_SPELL4]) 
			: str_empty);
	    break;
        ...
        case ITEM_WAND:
	    fprintf(fp, "%d %d %d '%s'\n",
	    		pObjIndex->value[ITEM_WAND_LEVEL],
	    		pObjIndex->value[ITEM_WAND_CHARGES_TOTAL],
	    		pObjIndex->value[ITEM_WAND_CHARGES_REMAINING],
	    		pObjIndex->value[ITEM_WAND_SPELL] != -1 
				? skill_name(pObjIndex->value[ITEM_WAND_SPELL]) 
				: str_empty);
	    break;
	case ITEM_PORTAL:
	    fprintf(fp, "%d %s %s %d %d\n",
			pObjIndex->value[ITEM_PORTAL_CHARGES],
	    		format_flags(pObjIndex->value[ITEM_PORTAL_EXIT_FLAGS]),
	    		format_flags(pObjIndex->value[ITEM_PORTAL_FLAGS]),
	    		pObjIndex->value[ITEM_PORTAL_DEST],
	    		pObjIndex->value[ITEM_PORTAL_KEY]);
	    break;

	case ITEM_LIGHT:
	    fprintf(fp, "%d\n",
			pObjIndex->value[ITEM_LIGHT_DURATION]);
	    break;
    ...
    fprintf(fp, "G %s\n", flag_string(rsex_flags, pObjIndex->rsex));
    fprintf(fp, "G %s\n", flag_string(rrace_flags, pObjIndex->rrace));
    fprintf(fp, "G %s\n", flag_string(rclass_flags, pObjIndex->rclass));
    fprintf(fp, "G %s\n", flag_string(rsize_flags, pObjIndex->rsize));
}
...
void save_room(FILE *fp, ROOM_INDEX_DATA *pRoomIndex)
{
	...
	for (door = 0; door < max_door; door++) {
		...
		if (pExit->to_room.r) {
 			...
 			||  IS_SET(pExit->rs_flags, EX_TRAPPED)
 			||  IS_SET(pExit->rs_flags, EX_SECRET)
 			||  IS_SET(pExit->rs_flags, EX_NOCLOSE)
...
void save_helps(FILE *fp, AREA_DATA *pArea)
{
	...
	for (; pHelp; pHelp = pHelp->next_in_area) {
		fprintf(fp, "%d %d %s~\n",
			pHelp->level, pHelp->clan, fix_string(pHelp->keyword));
...
/* save clan data */
	...
	for (i = 0; i < clan->skills.nused; i++) {
		...
		fprintf(fp,"Title %d male %s~\n",
			i, clan->rank_table[i].title[SEX_MALE]);
		fprintf(fp,"Title %d female %s~\n",
			i, clan->rank_table[i].title[SEX_FEMALE]);
	}
...
/* save plists */
	...
	/* 
	 * This needs to be kept in sync with clan.h clan.c and db/db_clan.c
	 * Zz
	 */
	fwrite_string(fp, "Patrons", clan->member_list[CLAN_PATRON]);
	fwrite_string(fp, "Leaders", clan->member_list[CLAN_LEADER]);
	fwrite_string(fp, "Elders", clan->member_list[CLAN_ELDER]);
	fwrite_string(fp, "Lords", clan->member_list[CLAN_LORD]);
	fwrite_string(fp, "Magistrates", clan->member_list[CLAN_MAGISTRATE]);
	fwrite_string(fp, "Ambassadors", clan->member_list[CLAN_AMBASSADOR]);
	fwrite_string(fp, "Vassals", clan->member_list[CLAN_VASSAL]);
	fwrite_string(fp, "Captains", clan->member_list[CLAN_CAPTAIN]);
	fwrite_string(fp, "Lieutenants", clan->member_list[CLAN_LIEUTENANT]);
	fwrite_string(fp, "Veterans", clan->member_list[CLAN_VETERAN]);
	fwrite_string(fp, "Soldiers", clan->member_list[CLAN_SOLDIER]);
	fwrite_string(fp, "Recruits", clan->member_list[CLAN_RECRUIT]);
	fwrite_string(fp, "Probations", clan->member_list[CLAN_PROBATION]);
	fwrite_string(fp, "Outcasts", clan->member_list[CLAN_OUTCAST]);
...
/* materials ***********************************************************/
void save_materials(CHAR_DATA *ch)
{
	FILE *fp;
	int i;
...
*/
	...
	if ((fp = dfopen( ETC_PATH, MATERIALS_CONF, "w")) == NULL) {
		save_print(ch, "%s%c%s: %s",
			ETC_PATH, PATH_SEPARATOR, MATERIALS_CONF,
			strerror(errno));
		return;
	}
...
void save_race(CHAR_DATA *ch, race_t *race, bool message)
{
    ...
    fprintf(fp, "Dodge %d\n",    race->dodge);
    if (race->alignment)fprintf(fp, "Align %d\n",	race->alignment);
    if (race->sex)	fprintf(fp, "Sex %s\n",		flag_string(sex_table, race->sex));
    if (race->size)	fprintf(fp, "Size %s\n",	flag_string(size_table, race->size));

    if (race->dam_type) fprintf(fp, "DamType %s\n",	attack_table[race->dam_type].name);
...
void do_asave(CHAR_DATA *ch, const char *argument)
{
	...
	/* ------------------------------------------ */
	if (!str_prefix("changed", argument)) {
	...
	if (!str_prefix("materials", argument)) {
		save_materials(ch);
		return;
	}
