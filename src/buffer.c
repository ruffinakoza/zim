/*
 * $Id: buffer.c 881 2006-06-26 02:45:14Z zsuzsu $
 */
...
#define BUF_LIST_MAX		15
#define BUF_DEFAULT_SIZE 	16384
...
static bool buf_cat(BUFFER *buffer, const char *string)
{
	...
	if (len >= buffer->size) { /* increase the buffer size */
		...
		if (buffer->size == -1) { /* overflow */
			...
			log_printf("buf_add: '%s': buffer overflow (%u) UINT_MAX=%u",
				string, oldsize, UINT_MAX);
			return FALSE;
		}
	}
